        ...
        try {
            ...
            if (owner_id.length > 0){
                ...
                try {
                    const [selectAward] = await connection.query('SELECT id FROM awards WHERE owner = UUID_TO_BIN(?) AND id = ? AND award_name = ?', [owner_id[0].user_id, award_id, award_name])
                    if (selectAward.length > 0) {
                        const [selectQuestions] = await connection.query('SELECT BIN_TO_UUID(id) as id, question, question_type, url, mandatory, question_choices, description FROM questions WHERE id_award = ? ORDER BY order_id', [selectAward[0].id])
        ...
        try {
            ...
            if (owner_id.length > 0){
                ...
                if (selectAward.length > 0) {
                    ...
                    const [selectQuestions] = await connection.query('SELECT BIN_TO_UUID(id) as id, question, question_type, url, mandatory, question_choices, description FROM questions WHERE id_award = ? ORDER BY order_id' , [award_id])
                    ...
                    if (selectQuestions.length > 0) { // En caso de que haya questions
                        ...
                        for (let i in questions) {
                            ...
                            for (let x in selectQuestions) {
                                ...
                                if (questionInputId.id === questionDbId.id) {
                                    await connection.query('UPDATE questions SET question = ?, question_type = ?, order_id = ?, url = ?, mandatory = ?, question_choices = ?, description = ? WHERE id = UUID_TO_BIN(?)', [questionInputId.question, questionInputId.question_type, questionInputId.order_id, questionInputId.url, questionInputId.mandatory, questionInputId.question_choices, questionInputId.description, questionInputId.id])
                                    changed = true
                                    break
                                } 
                            }
                            ...
                            if (!changed) {
                                console.log ("ohola");
                                await connection.query('INSERT INTO questions (id_award, question, question_type, order_id, url, mandatory, question_choices, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [award_id, questionInputId.question, questionInputId.question_type, questionInputId.order_id, questionInputId.url, questionInputId.mandatory, questionInputId.question_choices, questionInputId.description])
                            }
                        }
                        ...
                        for (let i in questions) {
                            ...
                            await connection.query('INSERT INTO questions (id_award, question, question_type, order_id, url, mandatory, question_choices, description) VALUES (?, ?, ?, ?, ?, ?), ?', [award_id, question.question, question.question_type, question.order_id, question.url, question.mandatory, question.question_choices, question.description])
                        }
